...
- **Const / DAO / DTO / Servlet / JSP** の構成。**Logic（ビジネスロジック専用）クラスは作らず**、
  業務ロジックは各 Servlet に実装しています。
- URL・パラメータ名・属性名・固定メッセージは **`WebConst` 定数クラスに集約**し、
  サーブレットは `import static com.ibm.eastb.harumi.common.WebConst.*` で参照します
  （文字列リテラルの散在を防止）。
- `WebConst` の各定数は **`BindConstContextListener`（`@WebListener`＋リフレクション）**
  によって ServletContext 属性として公開され、JSP から `${URL_ORDER_SERVLET}` のように
  EL で参照できます。
- 画面遷移リンク・リダイレクトは **`contextPath` を使わずコンテキストルートからの相対パス**
  で統一しています（`href="${URL_MENU_SERVLET}"` / `response.sendRedirect("order")` 等）。
- DAO クラスは `Dao` サフィックス・**static メソッド**で統一（例：`ItemDao.findOnSale()`）。
...
├── common/ … Web層の共通部品
│   ├── WebConst.java                （URL・パラメータ名・属性名・固定メッセージの定数）
│   └── BindConstContextListener.java（WebConst定数をEL用にServletContextへ公開）
├── dao/    … DBアクセス（Daoサフィックス・staticメソッド）
